...
<?php echo Form::open(["method" => "post","url" => action("PublicationController@create"), "onsubmit" => "getValues(event)","files" => true])?>
			...
			<div id="right-sidebar">
				...
				<div id="options">
					<div class="option"><?php echo Form::submit("Crear", ["class" => "btn option"]) ?></div>
				</div>

				<?php echo Form::hidden("type","post") ?>
				<?php if(Auth::user()->type == "admin"){ ?>
				<label><?php echo trans("messages.o-types") ?></label>
				<a href=<?php echo route("game-new") ?>><?php echo trans("messages.game") ?></a>
				<?php } ?>
			</div>
<?php echo Form::close()?>